...
JOB FILE FORMAT (INI-like):
  [job]
  id=server-notroot
  remote=notroot@server
  remote_root=~
  stage_dir=/var/tmp/ilma/stage/server-notroot
  target_dir=/backups/server/notroot
  mode=backup,archive
  verify=true
  cleanup_stage=false
  basename=server-notroot
...
It is stored locally (e.g. nodes/server/notroot.ini) and fed directly to rsync.
